import json
import random
from collections import defaultdict
from pathlib import Path

import pandas as pd
import torch

from autocomod.graph_nn import GnnEncoder, ProjectionHead
from autocomod.graph_nn.module_classification_pipeline import (
    GnnModuleClassificationPipeline,
)
from autocomod.graphs_dataset import GraphsDataset
from autocomod.logger import logger
from autocomod.settings import Settings


class ModuleClassificationSettings(Settings):
    experiment_name: str = "module_classification_gnn"
    repos_file: str = "data/repos.csv"
    pyg_dir: str = "data/pyg_graphs"
    checkpoints_dir: str = "checkpoints"
    continue_training: bool = True
    num_epochs: int = 3000
    learning_rate: float = 1e-3
    hidden_dim: int = 128
    out_dim: int = 64
    validation_frequency: int = 10
    num_repos: int | None = None


def train_module_classification(settings: ModuleClassificationSettings):
    experiment_dir = Path(settings.checkpoints_dir) / settings.experiment_name
    experiment_dir.mkdir(parents=True, exist_ok=True)

    checkpoint_path = experiment_dir / "model.pt"
    train_metrics_path = experiment_dir / "train.json"
    validation_metrics_path = experiment_dir / "validation.json"

    repos = pd.read_csv(settings.repos_file)
    all_files = [f"{repo.id}.pt" for _, repo in repos.iterrows()]

    pyg_dir = Path(settings.pyg_dir)
    existing_files = [f for f in all_files if (pyg_dir / f).exists()]

    if settings.num_repos is not None and settings.num_repos < len(existing_files):
        random.seed(42)
        existing_files = random.sample(existing_files, settings.num_repos)

    dataset = GraphsDataset(settings.pyg_dir, existing_files)
    logger.info(f"Loaded {len(dataset)} graphs from {settings.pyg_dir}")

    in_dim = dataset[0].x.shape[1]

    model = GnnEncoder(
        in_channels=in_dim, hidden=settings.hidden_dim, out_channels=settings.out_dim
    )
    projector = ProjectionHead(in_dim=settings.out_dim, proj_dim=settings.out_dim)

    optimizer = torch.optim.Adam(
        list(model.parameters()) + list(projector.parameters()),
        lr=settings.learning_rate,
    )

    start_epoch = 1
    train_history = defaultdict(list)
    validation_history = defaultdict(list)

    if settings.continue_training and checkpoint_path.exists():
        logger.info(f"Loading checkpoint from {checkpoint_path}")
        pipeline = GnnModuleClassificationPipeline.load(
            str(checkpoint_path), model, projector, optimizer
        )

        if train_metrics_path.exists():
            with open(train_metrics_path) as f:
                train_history = defaultdict(list, json.load(f))
                start_epoch = len(train_history["epoch"]) + 1

        if validation_metrics_path.exists():
            with open(validation_metrics_path) as f:
                validation_history = defaultdict(list, json.load(f))

        logger.info(f"Resuming training from epoch {start_epoch}")
    else:
        pipeline = GnnModuleClassificationPipeline(model, projector, optimizer)
        logger.info("Starting training from scratch")

    for epoch in range(start_epoch, settings.num_epochs + 1):
        train_metrics = pipeline.train_epoch(dataset)

        train_history["epoch"].append(epoch)
        for key, value in train_metrics.items():
            train_history[key].append(value)

        if epoch % settings.validation_frequency == 0:
            logger.info(f"Training metrics at epoch {epoch}: {dict(train_metrics)}")

            val_metrics = pipeline.validate_epoch(dataset)

            validation_history["epoch"].append(epoch)
            for key, value in val_metrics.items():
                validation_history[key].append(value)

            logger.info(f"Validation metrics at epoch {epoch}: {dict(val_metrics)}")

        pipeline.save(str(checkpoint_path))
        with open(train_metrics_path, "w") as f:
            json.dump(dict(train_history), f, indent=2)
        with open(validation_metrics_path, "w") as f:
            json.dump(dict(validation_history), f, indent=2)

    logger.info(f"Training completed. Checkpoint saved at {checkpoint_path}")


if __name__ == "__main__":
    train_module_classification(settings=ModuleClassificationSettings())
